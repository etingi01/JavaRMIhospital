...
public class WarningsRepository {
	  ...
	  public static ArrayList<Warnings> findByCodeConsultation(int criteria) throws ClassNotFoundException {
		  ArrayList<Warnings> arr = new ArrayList();
		    try {
		      String QRY = "SELECT * FROM warnings WHERE codeConsultation=?";
		      Connection con = DBManager.getConnection();
		      PreparedStatement pstmt = con.prepareStatement(QRY);
		      pstmt.setInt(1,  criteria );
		      ResultSet rs = pstmt.executeQuery();
	  ...
	  public static ArrayList<Warnings> findPrimaryAll() throws ClassNotFoundException {
		  ArrayList<Warnings> arr = new ArrayList();
		    try {
		      String QRY = "SELECT * FROM warnings";
		      Connection con = DBManager.getConnection();
		      PreparedStatement pstmt = con.prepareStatement(QRY);
		      ResultSet rs = pstmt.executeQuery();
		      while (rs.next()) {
